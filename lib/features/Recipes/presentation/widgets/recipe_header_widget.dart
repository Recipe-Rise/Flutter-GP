...
class _RecipeHeaderWidgetState extends State<RecipeHeaderWidget> {
  ...
  Future<void> _addCalories() async {
    ...
    // Add calories to consumed calories
    await _cacheHelper.addConsumedCalories(recipeCalories);

    // Get updated data for display
    final double bmr = double.tryParse(AppCubit.get(context).getUserData?.bmr ?? '0') ?? 0.0;
    final double remainingCalories = await _cacheHelper.getRemainingCalories(bmr);
    final double consumedCalories = await _cacheHelper.getConsumedCalories();

    Fluttertoast.showToast(
      msg: 'Calories added! Consumed: ${consumedCalories.toInt()}, Remaining: ${remainingCalories.toInt()}',
        ...
        child: Column(
          ...
          children: [
                    ...
                    showDialog(
                      ...
                      builder: (BuildContext context) {
                        return AlertDialog(
                          title: const Text('Confirm Action'),
                          content: Text(
                            'This recipe contains ${widget.recipe.calories?.toInt() ?? 0} calories. '
                                'Do you want to add it to your daily calorie intake?',
                          ),
                          actions: [
                            TextButton(
                              onPressed: () {
                            ...
                            TextButton(
                              onPressed: () async {
                                final double bmr = double.tryParse(AppCubit.get(context).getUserData?.bmr ?? '0') ?? 0.0;
                                final double currentConsumed = await _cacheHelper.getConsumedCalories();
                                final double recipeCalories = widget.recipe.calories ?? 0.0;
                                final double totalAfterRecipe = currentConsumed + recipeCalories;

                                if (totalAfterRecipe > bmr) {
                                  // Show warning if this would exceed daily calorie goal
                                  final double excess = totalAfterRecipe - bmr;

                                  showDialog(
                                    context: context,
                                    builder: (BuildContext context) {
                                      return AlertDialog(
                                        title: const Text('Exceeds Daily Goal'),
                                        content: Column(
                                          mainAxisSize: MainAxisSize.min,
                                          children: [
                                            const Icon(
                                            ...
                                            const SizedBox(height: 16),
                                            Text(
                                              'Adding this recipe (${recipeCalories.toInt()} calories) will exceed your daily goal by ${excess.toInt()} calories.\n\n'
                                                  'Current: ${currentConsumed.toInt()} / ${bmr.toInt()} calories\n'
                                                  'After recipe: ${totalAfterRecipe.toInt()} / ${bmr.toInt()} calories',
                                            ),
                                          ],
                                        ),
                                        actions: [
            ...
            Row(
              children: [
                _MetaItem(
                  text: '⏱️  ${widget.recipe.minutes} min',
                ),
                const SizedBox(width: 20),
                _MetaItem(
                  text: '🔥  ${widget.recipe.calories?.toInt() ?? 0} cal',
                ),
              ],
            ),
          ],
  ...
  void _showAddToHistoryDialog() {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
          title: const Text('Add Recipe to History'),
          content: const Text('Do you want to add this recipe to your cooking history?'),
          actions: [
            TextButton(
              onPressed: () {
                Navigator.of(context).pop();
