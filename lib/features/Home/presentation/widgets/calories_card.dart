...
class CaloriesCard extends StatelessWidget {
  ...
  Future<Map<String, double>> _fetchCalorieData() async {
    ...
    return await cacheHelper.getCalorieData(bmr);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<Map<String, double>>(
      future: _fetchCalorieData(),
      builder: (context, snapshot) {
        if (!snapshot.hasData) {
          return Container(
            width: double.infinity,
            height: screenSize.width * 0.45, // Fixed height to prevent layout shifts
            decoration: BoxDecoration(
              color: Colors.white,
              borderRadius: BorderRadius.circular(24),
              boxShadow: [
            ...
            child: const Center(
              child: CircularProgressIndicator(
                valueColor: AlwaysStoppedAnimation(Color(0xFF96B2FE)),
              ),
            ),
          );
        }
        ...
        final double remainingCalories = data['remaining']!;
        final double consumed = data['consumed']!;
        final double completionPercentage = data['completionPercentage']!;
